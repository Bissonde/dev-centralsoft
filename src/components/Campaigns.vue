<template>
    ...
    <v-container class="mx-auto mb-0 py-0" elevation="12" fluid style="width: 100%">
        ...
        <v-sheet class="pa-4 pb-0 mt-3" style="position: -webkit-sticky; position: sticky; top: 50px; z-index: 1"
            elevation="1" height="auto" width="100%">
            ...
            <v-row v-if="devType == 'Desktop'">
                <v-col cols="12" class="pb-5 mb-0 pl-8 pt-0">
                    <h1 class="text-h6 text-md-h5 font-weight-bold mb-1"><v-icon
                            class="text-h3">mdi-account-voice</v-icon>&nbsp;Campanhas<v-btn color="black" variant="text"
                            class="text-h6 pr-0" density="default" @click="goPlayModal = !goPlayModal; GoPlayNew();"
                            icon="mdi-pencil">
                        </v-btn>
                    </h1>
            ...
            <v-row cols="12" sm="6" md="6" lg="6" class="pt-0 mt-0" v-if="devType == 'Desktop'">
                <v-sheet class="d-flex flex-wrap pl-3 pr-3 ma-6 pt-0 mt-0" style="align-items: center;">
                    <div v-for="(n, index) in tabMenus.slice(0, 6)" :key="n">
                        <v-row align="center" justify="center" class="pb-0" dense>

                            <!-- <v-col cols="12" md="12"> -->
                            <v-hover v-slot="{ isHovering, props }">
                                <v-card :href="n.to" :color="n.bgcolor"
                                    class="mx-auto justify-center pa-2 mr-4 mb-4 rounded-0" width="202" max-width="202"
                                    height="82" border="start" variant="flat" hover>
            ...
            <v-row cols="12" sm="6" md="6" lg="6" class="pt-0 mt-2" v-if="devType != 'Desktop'">
                <v-sheet class="d-flex flex-wrap ml-0 ma-3 pa-1 pt-0 mt-0 justify-center" style="align-items: center;">
                    <div v-for="(n, index) in tabMenus-10 " :key="n">
                        <v-row align="center" justify="center" dense>

                            <!-- <v-col cols="12" md="12"> -->
                            <v-hover v-slot="{ isHovering, props }">
                                <v-card :href="n.to" :color="n.bgcolor" class="mx-auto pl-6 ml-3 mb-3 rounded-0"
                                    width="146" border="start" variant="flat" hover>
                                    <!-- <v-img :aspect-ratio="16 / 9"
                                                    src="https://cdn.vuetifyjs.com/images/cards/kitchen.png" cover>
                                                    <v-expand-transition>
                                                        <div v-if="isHovering"
        ...
        <v-sheet class="mt-5 " elevation="12" height="auto" width="100%">


            <v-row v-if="devType != 'Desktop'">
                <v-col cols="9">
                    <h1 class="text-h6 text-md-h5 font-weight-bold mb-1"><v-icon
                            class="text-h6">mdi-account-multiple</v-icon>&nbsp;Contactos
                    </h1>
                    <i class="float-sm-left text-body-2"><v-icon icon="mdi-clock-fast"></v-icon>&nbsp;
                        22/05/2023</i>
                </v-col>
            ...
            <v-data-table id="cTable" v-model:expanded="expanded" :headers="customeCampaignsHeaders"
                :items="serverItems" :search="search" :loading="loading"
                style="font-size: 10pt; width: 180%; table-layout: fixed; overflow-x:hidden; display: inline-block; height: auto;"
                item-value="surname" v-if="devType == 'Desktop'" show-select show-expand>
                <template v-slot:top>
                    <v-toolbar flat style="border-bottom:solid #005D40 5px; background-color: #fff">
                        ...
                        <v-toolbar-title>
                            ...
                            <v-text-field v-model="search" label="Procurar" prepend-inner-icon="mdi-magnify" clearable
                                width="400" density="compact" variant="outlined" hide-details
                ...
                <template v-slot:item.actions="{ item }">
                    <v-btn color="primary" class="me-2" variant="tonal" icon="mdi-play" density="compact"
                        @click="useModule.dialog = 'Campaigns'; useModule.title = 'Campanha'; editModal = !editModal; isEditing = false; CUSTOMER_CAMPAIGN_GET(item);"></v-btn>
                    ...
                    <v-btn color="bg-orange-darken-4" class="me-2" variant="text" icon="mdi-pencil-outline"
                        ...
                        @click="useModule.dialog = 'Campaigns'; useModule.title = 'Campanha'; editModal = !editModal; this.isEditing = true; CUSTOMER_CAMPAIGN_GET(item);"></v-btn>
    ...
    <v-dialog width='850' v-model="editModal" v-on:close="" v-on:open="beforeOpen" persistent>
        ...
        <v-card>
            ...
            <v-alert type="success" title="Sucesso" v-model="alertSuccess" transition="slide-y-transition"
                border="start" variant="tonal" closable text="As suas alterações foram gravadas com sucesso!"></v-alert>

            <v-alert type="warning" title="Sucesso" v-model="alertDelete" transition="slide-y-transition" border="start"
                variant="tonal" closable text="Registo removido com sucesso!"></v-alert>
          ...
          <span class="text-h5">User Profile</span>
                        ...
                        <v-card-text>
                            ...
                            <v-row dense fluid>
                                ...
                                <v-col cols="12" sm="6" md="4" class="pa-0 pb-0 mb-0">
                                    <v-text-field label="Primeiro nome*" id="FN" name="firstNameR"
                                        :disabled="!isEditing" v-model="firstName" clearable
                                        prepend-inner-icon="mdi-account" hint="*campo obrigatório"
                                        :error-messages="firstNameR.errorMessage.value" :counter="10"
                                ...
                                <v-col cols="12" sm="6" md="4" class="pa-0 pb-0 mb-0">
                                    <v-text-field :disabled="!isEditing" id="LN" name="lastNameR" :counter="10"
                                        ...
                                        label="Último nome*" required></v-text-field>
                                </v-col>
                                <v-col cols="12" sm="6" md="6" class="pa-0 pb-0 mb-0">
                                ...
                                <v-col cols="12" sm="6" md="6" class="pa-0 pb-0 mb-0">
                                    <v-text-field :disabled="!isEditing" id="cellphone" v-model="cellphone"
                                        label="Telefone*" type="telephone" prepend-inner-icon="mdi-phone"
                                        required></v-text-field>
                                </v-col>
                                <v-col cols="12" sm="5" md="6" class="pa-0 ml-0 pb-0 mb-0 pt-0 mt-0">
                                ...
                                <v-col cols="12" sm="6" md="6" class="pa-0 pb-0 mb-0 pt-0 mt-0">
                                    <v-text-field :disabled="!isEditing" id="cellEmail" v-model="cellEmail"
                                        label="E-mail*" prepend-inner-icon="mdi-email-fast-outline"
                                        required></v-text-field>
                                </v-col>
                                <v-col cols="12" sm="6" md="6" class="pa-0 pb-0 mb-0 pt-0 mt-0">
                                ...
                                <v-col cols="12" sm="6" md="6" class="pa-0 pb-0 mb-0 pt-0 mt-0">
                                    <v-autocomplete :disabled="!isEditing" v-model="cellCampaign"
                                        :items="cellCampaignList" prepend-inner-icon="mdi-forum"></v-autocomplete>
                                </v-col>
                                <v-col cols="12" sm="6" md="6" class="pa-0 pb-0 mb-0 pt-0 mt-0">
                    ...
                    <v-window-item v-model="options">
                        <v-card-text>
                            ...
                            <v-row>
                                ...
                                <v-col cols="12" sm="6" md="6" class="pa-0">
                                    <v-text-field :disabled="!isEditing" id="cellWhatsApp" v-model="cellWhatsApp"
                                        label="WhatsApp Empresa*" prepend-inner-icon="mdi-whatsapp"
                                        required></v-text-field>
                                </v-col>
                                <v-col cols="12" sm="6" md="6" class="pa-0">
                                    <v-autocomplete :disabled="!isEditing" v-model="cellLikes"
                                        ...
                                        label="Tipo de Cliente" prepend-inner-icon="mdi-set-center"
                                ...
                                <v-col cols="12" sm="6" md="6" class="pa-0">
                                    <v-text-field :disabled="!isEditing" id="cellphone" v-model="cellphone"
                                        label="Unidades Compradas" type="telephone"
                                        prepend-inner-icon="mdi-percent-box-outline" required></v-text-field>
                                </v-col>
                            </v-row>
                            <!-- </v-container> -->
                            <small>*indicates required field</small>
                            ...
                            <v-card-text>
                                ...
                                <v-row>
                                    ...
                                    <v-col cols="12" sm="12" md="6" class="pa-0">
                                        <v-text-field :disabled="!isEditing" label="Nome do Grupo*" id="GN"
                                            v-model="groupName" :counter="10" :rules="firstNameRules"
                                            prepend-inner-icon="mdi-account-multiple" required></v-text-field>
                                    </v-col>
                                    <v-col cols="12" sm="12" md="12" class="pa-0">
                                        <v-text-field :disabled="!isEditing" label="Descrição" id="GD"
                                            v-model="groupDescr" :counter="10" :rules="lastNameRules"
                                    ...
                                    <v-col cols="12" sm="6" md="6" class="pa-0">
                                        <v-autocomplete :disabled="!isEditing" :items="['Interno', 'Externo', 'Admin']"
                                            label="Tipo de Conta"
                                            prepend-inner-icon="mdi-account-badge-outline"></v-autocomplete>
                                    </v-col>
                                    <v-col cols="12" sm="6" md="6" class="pa-0">
                                    ...
                                    <v-col cols="12" sm="6" md="6" class="pa-0">
                                        <v-autocomplete :disabled="!isEditing"
                                            :items="['English', 'Português', 'François']" label="Linguagem"
                                            prepend-inner-icon="mdi-translate"></v-autocomplete>
                                    </v-col>
                                    <v-col cols="12" sm="6" md="6" class="pa-0">
                                        <v-autocomplete :disabled="!isEditing" :items="['Painel', 'Windows']"
                                            label="Modo de Abertura"
                                            prepend-inner-icon="mdi-open-in-new"></v-autocomplete>
                                    </v-col>
                                    <v-col cols="12" sm="6" md="6" class="pa-0">
                                        <v-autocomplete :disabled="!isEditing"
                        ...
                        <v-data-table :headers="groupMembersHeaders" :items="serverItems1" :search="search" show-select
                            ...
                            <template v-slot:top>
                                <v-toolbar flat style="border-bottom:solid #0277BD 5px;">
                                    ...
                                    <v-btn class="mb-2" color="primary" size="large" prepend-icon="mdi-cloud-refresh"
                                        dark @click="CUSTOMER_GET_ALL();">
                                    </v-btn>
          ...
          <span class="text-h5">User Profile</span>
                            ...
                            <v-card-text>
                                ...
                                <v-row>
                                    ...
                                    <v-col cols="12" sm="6" class="pa-0">
                                        <v-autocomplete :disabled="!isEditing" :items="cellContactList"
                                            label="Contactos" prepend-inner-icon="mdi-account"
                                            multiple></v-autocomplete>
                                    </v-col>
                                    <v-col cols="12" sm="6" md="6" class="pa-0">
                                        <v-text-field :disabled="!isEditing" label="Data de Início" type="date"
                                            v-model="campaignStart" prepend-inner-icon="mdi-calendar"
                                            required></v-text-field>
                                    </v-col>
                                    <v-col cols="12" sm="6" md="6" class="pa-0">
                                    ...
                                    <v-col cols="12" sm="12" class="pa-0">
                                        <v-autocomplete :disabled="!isEditing" v-model="campaignSms"
                                            :items="cellMessageList" :change="CUSTOMER_MESSAGE1_GET(campaignSms)"
                                            label="SMS"
                                            prepend-inner-icon="mdi-message-settings-outline"></v-autocomplete>
                                    </v-col>
                                    <v-col cols="12" sm="12" class="pa-0">
                                        <v-autocomplete :disabled="!isEditing" v-model="campaignEmail"
                                            :items="cellMessageList" label="E-mail"
                                            prepend-inner-icon="mdi-email-outline"></v-autocomplete>
                                    </v-col>
                                    <v-col cols="12" sm="12" class="pa-0">
                    ...
                    <v-window-item v-model="options">
                        <v-card>
                            <v-card-text>
                                ...
                                <v-list lines="three" select-strategy="classic">
                                    <v-list-subheader>General</v-list-subheader>
                                    <v-textarea :disabled="!isEditing" label="Mensagem*" v-model="msgText"
                                        :counter="200" :rules="lastNameRules"
                            ...
                            <v-card-text>
                                ...
                                <v-date-picker v-model="date" :allowed-dates="allowedDates" max="2028-03-20"
                                  min="2016-06-15"></v-date-picker>
                                    ...
                                    <v-col cols="12" sm="6" md="6" class="pa-0">
                                        <v-autocomplete :disabled="!isEditing" v-model="cellGroup"
                                            :items="cellGroupList" :item-value="cellGroupListID" label="Grupo"
                                            prepend-inner-icon="mdi-account-multiple"></v-autocomplete>
                                    </v-col>
                                    <v-col cols="12" sm="6" md="6" class="pa-0">
                                        <v-text-field :disabled="!isEditing" label="Título" v-model="msgTitle"
                                            :counter="10" :rules="firstNameRules"
                                            hint="example of helper text only on focus"
                                            prepend-inner-icon="mdi-format-title"></v-text-field>
                                    </v-col>
                                    <v-col cols="12" sm="12" md="12" class="pa-0">
          ...
          <span class="text-h5">User Profile</span>
                        ...
                        <v-card-text>
                            ...
                            <v-row>
                                ...
                                <v-col cols="12" sm="5" md="6" class="pa-0 ml-0">
                                    <v-text-field :disabled="!isEditing" label="SMS*"
                                        prepend-inner-icon="mdi-message-text" required></v-text-field>
                                </v-col>
                                <v-col cols="12" sm="6" md="6" class="pa-0">
                                    <v-text-field :disabled="!isEditing" label="E-mail*"
                                        prepend-inner-icon="mdi-password" required></v-text-field>
                                </v-col>
                                <v-col cols="12" sm="6" md="6" class="pa-0">
                                    <v-text-field :disabled="!isEditing" label="WhatsApp*"
                                        prepend-inner-icon="mdi-whatsapp" required></v-text-field>
                                </v-col>
                                <v-col cols="12" sm="6" md="6" class="pa-0">
                                ...
                                <v-col cols="12" sm="6" md="6" class="pa-0">
                                    <v-select :disabled="!isEditing" :items="['0-17', '18-29', '30-54', '54+']"
                                        label="Idade*" prepend-inner-icon="mdi-numeric-7" required></v-select>
                                </v-col>
                                <v-col cols="12" sm="6" md="6" class="pa-0">
                                    <v-autocomplete :disabled="!isEditing"
                                        :items="['Skiing', 'Ice hockey', 'Soccer', 'Basketball', 'Hockey', 'Reading', 'Writing', 'Coding', 'Basejump']"
                                        label="Interesses" prepend-inner-icon="mdi-set-center"
                                        multiple></v-autocomplete>
                                </v-col>
                                <v-col cols="12" sm="6" md="6" class="pa-0">
                                    <v-autocomplete :disabled="!isEditing"
                                        :items="['Marketing', 'Autenticação', 'Cobrança']" label="Campanha"
                                        prepend-inner-icon="mdi-forum" multiple></v-autocomplete>
                                </v-col>
                                <v-col cols="12" sm="6" md="6" class="pa-0">
                                ...
                                <v-col cols="12" sm="6" md="6" class="pa-0">
                                    <v-autocomplete :disabled="!isEditing" :items="['Cliente', 'Empresa', 'Interno']"
                                        label="Grupo" prepend-inner-icon="mdi-account-multiple"
                                        multiple></v-autocomplete>
                                </v-col>
                                <v-col cols="12" sm="6" md="6" class="pa-0">
                                    <v-autocomplete :disabled="!isEditing"
                            ...
                            <v-row>
                                <v-col cols="12" sm="6" md="4" class="pa-0">
                                    <v-form fast-fail @submit.prevent>
                                        <v-text-field label="Primeiro nome*" name="fname" v-validate="required"
                                            :disabled="!isEditing" v-model="firstName" :counter="10"
                                            :rules="firstNameRules" prepend-inner-icon="mdi-account"
                                            required></v-text-field>
                                    </v-form></v-col>
                                <v-col cols="12" sm="6" md="4" class="pa-0">
                                    <v-text-field :disabled="!isEditing" label="Nome do meio"
                                ...
                                <v-col cols="12" sm="6" md="4" class="pa-0">
                                    <v-text-field :disabled="!isEditing" v-model="lastName" :counter="10"
                                        :rules="lastNameRules" label="Último nome*" hint="*campo obrigatório"
                                ...
                                <v-col cols="12" sm="6" md="6" class="pa-0">
                                    <v-autocomplete :disabled="!isEditing" :items="['Homem', 'Mulher']"
                                        label="Município *" prepend-inner-icon="mdi-set-center"></v-autocomplete>
                                </v-col>
                                <v-col cols="12" sm="6" md="6" class="pa-0">
                    ...
                    <v-window-item v-model="service">
                        <v-card-text>
                            <v-row>
                                ...
                                <v-col cols="12" sm="6" md="6" class="pa-0">
                                    <v-text-field :disabled="!isEditing" v-model="lastName" :counter="20"
                                        :rules="lastNameRules" label="Número de Cama" persistent-hint
                                        required></v-text-field>
                                </v-col>
                                <v-col cols="12" sm="6" md="6" class="pa-0">
                            ...
                            <v-row>
                                <v-col cols="12" sm="6" md="6" class="pa-0">
                                    <v-autocomplete :disabled="!isEditing"
                                        ...
                                        label="Número de Hemotransfusões"
                                        prepend-inner-icon="mdi-set-center"></v-autocomplete>
                                </v-col>
                                <v-col cols="12" sm="6" md="6" class="pa-0">
                                    <v-autocomplete :disabled="!isEditing" :items="[
                    ...
                    <v-window-item v-model="details">
                        <v-card-text>
                            <v-row>
                                ...
                                <v-col cols="12" sm="6" md="6" class="pa-0">
                                    <v-autocomplete :disabled="!isEditing"
                                        :items="['Emanuel Catumbela', 'Nelito Catumbela']" label="Médico *"
                                        prepend-inner-icon="mdi-set-center"></v-autocomplete>
                                </v-col>
                                <v-col cols="12" sm="6" md="6" class="pa-0">
                                    <v-text-field :disabled="!isEditing" label="Data*" type="date"
...
export default {
    data: () => ({
        ...
        leftMenuItems: [
            { text: 'Home', icon: 'mdi-view-dashboard', to: '/dashboard' },
            { text: 'Contactos', icon: 'mdi-account', to: 'dashboard?nav=contacts', bgcolor: 'red' },
            { text: 'Grupos', icon: 'mdi-account-group', to: 'dashboard?nav=groups', bgcolor: 'red' },
            { text: 'Campanhas', icon: 'mdi-account-voice', to: 'dashboard?nav=campaigns', bgcolor: 'red' },
            { text: 'Agendamento', icon: 'mdi-account-clock', to: 'dashboard?nav=calendar', bgcolor: 'red' },
            { text: 'Mensagem', icon: 'mdi-message-plus', to: 'dashboard?nav=message', bgcolor: 'red' },
            { text: 'Chamadas', icon: 'mdi-phone-settings', to: 'dashboard?nav=calls', bgcolor: 'red' },
            { text: 'WhatsApp', icon: 'mdi-whatsapp', to: 'dashboard?nav=whatsapp', bgcolor: 'red' },
            { text: 'E-mail', icon: 'mdi-email-fast-outline', to: 'dashboard?nav=email', bgcolor: 'red' },
            { text: 'Histórico', icon: 'mdi-history', to: 'dashboard?nav=history', bgcolor: 'red' },
            { text: 'Configuarções', icon: 'mdi-cog', to: 'dashboard?nav=settings', bgcolor: 'red' },
        ],
        group: null,
        customerLogs: [
            {
        ...
        contactHeaders: [
            ...
            { title: 'Grupo', align: 'start', key: 'groupID' },
        ...
        tabMenus: [
            ...
            { text: 'Contactos', icon: 'mdi-account', to: 'dashboard?nav=contacts', descr: 'Directório de Contactos', dialog: 'Contacts', size: '400px', bgcolor: '#001D40' },
            { text: 'Grupos', icon: 'mdi-account-group', to: 'dashboard?nav=groups', descr: 'Directório de Grupos de Contactos', dialog: 'Groups', size: '400px', bgcolor: '#002D41' },
            { text: 'Campanhas', icon: 'mdi-account-voice', to: 'dashboard?nav=campaigns', descr: 'Campanhas', dialog: 'Campaigns', bgcolor: '#003D42' },
            // { text: 'Agendamento', icon: 'mdi-account-clock', to: 'calendar', descr: 'Agendamento', dialog: 'Booking' },
            { text: 'Mensagens', icon: 'mdi-message-plus', to: 'dashboard?nav=message', descr: 'Mensagens', dialog: 'Messages', bgcolor: '#004D43' },
            { text: 'Chamadas', icon: 'mdi-phone-settings', to: 'dashboard?nav=calls', descr: 'Chamadas Automaticas', dialog: 'Chamadas', bgcolor: '#005D44' },
            { text: 'WhatsApp', icon: 'mdi-whatsapp', to: 'dashboard?nav=whatsapp', descr: 'Enviar mensagens via WhatsApp', dialog: 'WhatsApp', bgcolor: '#006D45' },
            { text: 'E-mail', icon: 'mdi-email-fast-outline', to: 'dashboard?nav=email', descr: 'E-mails', dialog: 'E-mail', bgcolor: '#007D46' },
            { text: 'Histórico', icon: 'mdi-history', to: 'dashboard?nav=history', descr: 'Acesse ao hsitóricos de operações', dialog: 'Histórico', bgcolor: '#008D47' },
    ...
    watch: {
        allContacts(newValue, oldValue) {
            ...
            if (newValue.length > 0 & this.loading == true) {
                ...
                if(this.allcontacts.length > 1 && table.offsetHeight > 300){
                this.overlayOFF();
                }
            }
        },
    ...
    methods: {
        ...
        CUSTOMER_CAMPAIGN_GET_ALL: async function () {
            ...
            useModule.dialog = 'Campaign'
            useModule.name = 'Campanha'
            useModule.text = 'Campanha'
