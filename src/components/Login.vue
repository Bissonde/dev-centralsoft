    ...
    <v-form ref="form" fast-fail @submit.prevent="submit" class="circular"
        ...
        <div class="d-flex align-center justify-center" width="600" max-width="456" v-if="alert == 'pwd'"
            ...
            <v-card id="rstPwd" class="pa-5 pb-8 ma-2" elevation="8" width="500" max-width="448" rounded="sm"
                ...
                <v-alert v-model="resetPwdOk" border="start" variant="tonal" closable close-label="Close Alert"
                    color="warning" title="Atenção!" type="warning">
                ...
                <v-card-text class="text-center">
                    ...
                    <v-row>
                        ...
                        <v-col cols="md-6">
                            <v-btn type="submit" @click="AuthLogin()" rounded="0" block class="mb-0"
                                color="blue-darken-4" size="large" v-if="alert == 'pwd'" variant="flat">
                    ...
                    .then(
                        (response) => {
                            ...
                            if (response.data.token != undefined) {
                                ...
                                var act = JSON.stringify(authx);
                                const joke = axios.get('Account/' + PWE)
                                    .then(
                                        (response) => {
                                            // window.location = '/profile'
                                            // alert(response.request.status)
